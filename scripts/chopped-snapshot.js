...
const LEAGUE_ID = "1390614263638265856";
...
function isRelevantPosition(p) {
  ...
  return fp.includes('QB') || fp.includes('RB') || fp.includes('WR') || fp.includes('TE');
}
...
async function main() {
  ...
  fs.writeFileSync(path.join(outDir, 'chopped-pool.json'), JSON.stringify(snapshot, null, 2));
